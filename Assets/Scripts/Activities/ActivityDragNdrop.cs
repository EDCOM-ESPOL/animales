...
public class ActivityDragNdrop : MonoBehaviour {
...
//        txtShowOrder.text = "Puntaje: " + score+ " / Orden: Utilidad" + " / Respuesta: " + ((Animal.utility)answer).ToString();
        ...
        for (int i=0 ; i<3; i++){
			        newAnimals.Add(AnimalManager.Instance.GetRandomAnimalByUtility(i,modo));
					newAnimals.Add(AnimalManager.Instance.GetRandomAnimalByUtility(i,modo));
					newAnimals.Add(AnimalManager.Instance.GetRandomAnimalByUtility(i,modo));
                    newAnimals.Add(AnimalManager.Instance.GetRandomAnimalByUtility(i,modo));
		}
        ...
        foreach (Animal animal in newAnimals)
        {
            ...
            newAnimalOption.GetComponent<AnimalOptionDisplay>().animal = animal;
                        newAnimalOption.AddComponent<BoxCollider2D>();
                        newAnimalOption.AddComponent<DragNdrop>();
...
//                oldCB.pressedColor = myColors[1];
      ...
      //      oldCB.disabledColor = myColors[0];
            ...
            //BoxCollider2D sc = newAnimalOption.AddComponent(typeof(BoxCollider)) as BoxCollider2D;
...
//            oldCB.disabledColor = myColors[1];
  //          buttonClicked.colors = oldCB;
            ...
            int[] scores = SessionManager.Instance.getPlayerScore();//////aquiiiiiiiiiiiiii
            //bool[] levels = SessionManager.Instance.getLevels();

            
                     scores[0] = scores[0] + 1; //aquiiii
                    score++;
                    Debug.Log(score);
            ...
            if (subLevelFinished)
            {
                SessionManager.Instance.setPlayerScore(scores);///////aquiiiiiiiiiiiiiiiiiiiii
